...
zfs_uio_set_pages_to_stable(zfs_uio_t *uio)
{
	...
	for (int i = 0; i < uio->uio_dio.num_pages; i++) {
		...
		MPASS(page == PHYS_TO_VM_PAGE(VM_PAGE_TO_PHYS(page)));
		vm_page_busy_aquire(page, VM_ALLOC_SBUSY);
		pmap_remove_write(page);
	}
}
...
zfs_uio_iov_step(struct iovec v, zfs_uio_t *uio, int *numpages)
{
	...
	res = zfs_uio_get_user_pages(addr, n, zfs_uio_rw(uio),
...
{
	int error = 0;
	size_t n_pages = DIV_ROUND_UP(zfs_uio_resid(uio), PAGE_SIZE);

	ASSERT(zfs_uio_rw(uio) == rw);

	uio->uio_dio.pages = kmem_alloc(n_pages * sizeof (vm_page_t), KM_SLEEP);

	error = zfs_uio_get_dio_pages_impl(uio);

	if (error) {
		kmem_free(uio->uio_dio.pages, n_pages * sizeof (vm_page_t));
		return (error);
	}
