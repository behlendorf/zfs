...
dbuf_sync_leaf(dbuf_dirty_record_t *dr, dmu_tx_t *tx)
{
	...
	if (db->db_state == DB_READ) {
		 ...
		 * write and is currently transistioning the db_state from
		 * DB_READ in dbuf_read_impl() to another state in
		 * dbuf_read_done(). We have to wait in order for the dbuf state
		 * to change from DB_READ before syncing the dirty record of the
		 * Direct IO write.
		 */
		ASSERT3P(*datap, ==, NULL);
		ASSERT3P(db->db_buf, ==, NULL);
		...
		ASSERT(db->db_state == DB_CACHED ||
		    db->db_state == DB_UNCACHED);
		IMPLY(db->db_state == DB_UNCACHED, db->db.db_data == NULL);
	} else if (db->db_state == DB_UNCACHED) {
