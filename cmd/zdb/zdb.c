...
/*
 * Prints the elements in each row properly for -P output.
 */
static inline void
prtrow_nicenum(uint64_t number, int width)
{
	char numbuf[32];
	...
	if (dump_opt['P'])
		...
		printf("%*s", width, numbuf);
}

/*
 * Used to simply reporting of the histogram data.
 */
...
dump_size_histograms(zdb_cb_t *zcb)
{
	 ...
	 */
	int minidx = SPA_MAXBLOCKSHIFT - 1;
	int maxidx = 0;
	uint64_t max = 0;
	...
	if (dump_opt['P']) {
		...
		(void) printf("\n%*s ", firstcolwidth, blocksize_title1);
	}
