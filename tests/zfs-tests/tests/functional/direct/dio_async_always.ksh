...
log_must fio --filename=$tmp_file --name=always-randwrite \
        --rw=randwrite --bs=$page_size --size=$file_size --numjobs=1 \
	--ioengine=libaio --fallocate=none --iodepth=4 --verify=sha1 \
        --group_reporting --minimal --runtime=$runtime --time_based

log_pass "Verify small async direct IO requests"
